...
import { clearCart, removePizzaCart, incPizzaItem, decPizzaItem } from '../redux/action/cart';
...
const Cart = () => {
	...
	const onClearCart = () => {
		if (window.confirm('Вы действительно хотите все удалить ?')) {
			dispatch(clearCart());
		}
	}
	...
	const onIncItem = (id) => {
		dispatch(incPizzaItem(id))
	}

	const onDecItem = (id) => {
		dispatch(decPizzaItem(id))
	}
